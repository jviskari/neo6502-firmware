...
static void test_sfx()
{
  ...
  NEO_Dispatch(NEO_CMD_SOUND, NEO_FN_PLAY_SOUND);
}
...
void SetCursorPosition(uint8_t x_pos, uint8_t y_pos)
{
  NEO.p0 = x_pos;
  NEO.p1 = y_pos;
  NEO_Dispatch(NEO_CMD_CONSOLE, NEO_FN_CURSOR_POS);
}
...
int write(int /* fildes */, const unsigned char *buf, unsigned count)
{
  while (count--)
  {
    ...
    NEO.p0 = *buf++;
    NEO_Dispatch(NEO_CMD_CONSOLE, NEO_FN_WRITE_CHAR);
  }
...
void drawLine_1(int x1, int y1, int x2, int y2)
{
  ...
  // Command 2, draw line
  // In group 5 - this write triggers the action.
  NEO_Dispatch(5, 2);
...
uint16_t NEO_Sin(uint16_t a)
{
  while (NEO_IsBusy())
  {
  } // Waits for the previous command to finish.

  NEO.math.reg = (uint16_t)a;

  NEO_Dispatch(NEO_CMD_MATH, NEO_FN_SIN);
...
void drawLine_3(int x1, int y1, int x2, int y2)
{
 ...
 void __fastcall__ _neo_draw_line(void)
{
  while (NEO_IsBusy())
  {
  } // Waits for the previous command to finish.
